...
namespace DeepeningPJ
{
    public class PlayerState : IState
    {
        ...
        public PlayerState(PlayerStateMachine playerMovementStateMachine)
        {
        ...
        protected void OnLayer(int layerIndex)
        {
            stateMachine.Player.Animator.SetLayerWeight(layerIndex, 1);
        }

        protected void OffLayer(int layerIndex)
        {
            stateMachine.Player.Animator.SetLayerWeight(layerIndex, 0);
        }

        protected bool GetAnimationParameter(int animationHash)
        {
            return stateMachine.Player.Animator.GetBool(animationHash);
        }

        protected bool IsInTransition()
        {
            return stateMachine.Player.Animator.IsInTransition(0);
        }
        ...
        {
            stateMachine.Player.OnPlayerDamaged += TakeDamage;

            StateMachineManager.onStateEnter += OnStateEnter;
            StateMachineManager.onStateUpdate += OnStateUpdate;
            StateMachineManager.onStateExit += OnStateExit;
        }

        protected virtual void RemoveEventMethods()
        {
            stateMachine.Player.OnPlayerDamaged -= TakeDamage;
        ...
        protected void RotateTowardsTargetRotation(bool isSmooth = true)
        {
            ...
            if (isSmooth)
            {
                ...
                targetRotation = Quaternion.Euler(0f, smoothYAngle, 0f);
            }
            else
            {
                targetRotation = Quaternion.Euler(0f, stateMachine.ReusableData.CurrentTargetRotation.y, 0f);
            }
